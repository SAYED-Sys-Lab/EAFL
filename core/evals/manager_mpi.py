...
def process_cmd(yaml_file, total_processes):
    ...
    if global_rank == 0:
        ...
        job_conf = {'time_stamp': time_stamp,
                    'total_worker': total_processes - 1,
                    'ps_ip': ps_ip,
        ...
        learner_conf = '-'.join([str(_) for _ in list(range(1, total_processes))])
        ...
        ps_cmd = f" python {yaml_conf['exp_path']}/aggregator.py {conf_script} --this_rank={global_rank} --learner={learner_conf} --cuda_device=cuda:{local_rank}"
        ...
        with open(f"{log_path}/all_logs", 'a') as fout:
            ...
            subprocess.call(f'{setup_cmd} {ps_cmd}', shell=True,  stdout=fout, stderr=subprocess.STDOUT)
    ...
    #Ahmed - put them in else condition to exclude the PS GPU from the worker allocations
        ...
        learner_conf = '-'.join([str(_) for _ in list(range(1, total_processes))])
        # =========== Submit job to each worker ============
        print(f"Starting worker {local_rank}:{global_rank} with PS: {job_conf['ps_ip']}...")
        ...
        worker_cmd = f" python {yaml_conf['exp_path']}/executor.py {conf_script} --this_rank={global_rank} --learner={learner_conf} --cuda_device=cuda:{local_rank}"
        with open(f"{log_path}/all_logs", 'a') as fout:
