...
def process_cmd(yaml_file, remote=False):
    ...
    if remote:
        submit_cmd = f'ssh {submit_user}{ps_ip} "{setup_cmd} {ps_cmd}"'
    else:
        submit_cmd = f'{setup_cmd} {ps_cmd}'
    print(f'{submit_cmd}')
    with open(f"{job_name}_logging", 'a') as fout:
        subprocess.Popen(f'{submit_cmd}', shell=True, stdout=fout, stderr=fout)
    ...
    for worker, gpu in zip(worker_ips, total_gpus):
        ...
        for cuda_id in range(gpu):
            ...
            if remote:
                submit_cmd = f'ssh {submit_user}{ps_ip} "{setup_cmd} {worker_cmd}"'
            else:
                submit_cmd = f'{setup_cmd} {worker_cmd}'
            print(f'{submit_cmd}')
...
if sys.argv[1] == 'submit':
    ...
    process_cmd(sys.argv[2], remote=False)
