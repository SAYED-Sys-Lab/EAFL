...
parser.add_argument('--stale_update', type=int, default=0) # number of rounds to wait for stale updates, 0 inactive
...
parser.add_argument('--deadline', type=float, default=0.0) # deadline in time for each round, -1 uses last worker time for mov_average, -2 uses targratio worker for mov_average
...
parser.add_argument('--random_behv', type=int, default=0) # assignment of behaviour profile to the clients, 0=sequential, 1=random, 2=assign among least availiable 10%
parser.add_argument('--total_clients', type=int, default=1000) # total number of clients to use in simulation, 0 uses the number clients in data map file
...
parser.add_argument('--filter_class_ratio', type=float, default=0) # ratio of classes/labels to randomly filter out (remove) per client
