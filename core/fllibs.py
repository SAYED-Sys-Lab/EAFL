...
#Ahmed - create custom dataset
# Ahmed - Reproducability - introduce a per-item seed as in (https://discuss.pytorch.org/t/large-differences-in-training-runs-depending-on-worker-number/43352/3)
# Pytorch issue (https://github.com/pytorch/pytorch/issues/7068)
class MyDataset(datasets.CIFAR10):
    ...
    def __getitem__(self, idx):
        ...
        data, target = super(MyDataset, self).__getitem__(idx)
        logging.info(f'idx:{idx} seeds: {seed} target:{target} datalen:{len(data)}')
        # seed = self.seeds[idx]
        # torch.manual_seed(seed)
        # data = self.data[idx]
...
def init_dataset():
    ...
    if args.task == "detection":
        ...
        if args.data_set == 'Mnist':
            ...
            #Ahmed - use the custom MyDataset class
            # Ahmed - Reproducability - wrappers to cutomize the __getitem__ function
        ...
        #Ahmed - existing dataset does not contain client mapping files
        ...
        # #Ahmed - Add reddit dataset
        # elif args.data_set == 'reddit':
        #     from utils.reddit import reddit
        #
        #     train_dataset = reddit(args.data_dir, train=True)
        #     test_dataset = reddit(args.data_dir, train=False)
