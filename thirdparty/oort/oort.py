...
class _training_selector(object):
    ...
    def __init__(self, args, sample_seed=233):
        ...
        self.rng = Random()
        #self.rng.seed(sample_seed + 233) #Ahmed - change to use increments 0,1,2 starting from default 233
        self.rng.seed(sample_seed) #use values of 0,1,2, ... instead
        self.unexplored = set()
        self.args = args
    ...
    def register_client(self, clientId, feedbacks):
        # Initiate the score for arms. [score, timef_stamp, # of trials, size of client, auxi, duration]
        if clientId not in self.totalArms:
            self.totalArms[clientId] = {}
            self.totalArms[clientId]['reward'] = feedbacks['reward']
            #self.totalArms[clientId]['energy'] = feedbacks['energy']
    ...
    def getTopK(self, numOfSamples, training_round, feasible_clients):
        ...
        #Ahmed - handle empty exploitation list
        self.exploitClients = []
        ...
        while num < numOfSamples:
            cid = np2.random.choice(orderedKeys)
